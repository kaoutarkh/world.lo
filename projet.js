(()=>{
    ...
    const addMenuBackground = () =>{
        ...
        const corps = document.documentElement.scrollTop;
        const navigation = document.querySelector("header nav");

        if(pageWidth > Width){
            corps > 0 ? navigation.classList.add("nav-fixed") :
    ...
    const reorderResponsiveMenu = () => {
        ...
        if(pageWidth <= Width && navigation){
            document.body.insertAdjacentElement("afterbegin",navigation);
        }
        else if(pageWidth > Width && mobileNavigation){
